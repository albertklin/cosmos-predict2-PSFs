...
"""Training wrapper for the multi-view action conditioned model."""
...
class Predict2MultiviewActionConditionedModel(Predict2Video2WorldModel):
    """Model used for fine-tuning a multi-view action conditioned world model."""
    ...
    def __init__(self, config: Predict2MultiviewModelConfig):
        ...
        self.pipe = MultiviewActionPipeline.from_config(
            config.pipe_config, dit_path=config.model_manager_config.dit_path
        )
    ...
    def forward(self, data_batch: dict, data_batch_idx: int):
